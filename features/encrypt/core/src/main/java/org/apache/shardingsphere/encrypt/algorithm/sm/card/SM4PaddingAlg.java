package org.apache.shardingsphere.encrypt.algorithm.sm.card;
...
public final class SM4PaddingAlg implements EncryptAlgorithm {
    ...
    public void init(final Properties props) {
        ...
        metaData = new EncryptAlgorithmMetaData(true, false, false,new Properties());
    }

    private int createSm4Mode(final Properties props) {
        ShardingSpherePreconditions.checkState(props.containsKey(MODE), () -> new AlgorithmInitializationException(this, String.format("%s can not be null", MODE)));
        String mode = String.valueOf(props.getProperty(MODE)).toUpperCase();
        ShardingSpherePreconditions.checkState(MODES.contains(mode), () -> new AlgorithmInitializationException(this, "Mode must be either CBC or ECB"));
        switch (mode) {
            case "CBC":
    ...
    private int createSm4Key(final Properties props) {
        ShardingSpherePreconditions.checkState(props.containsKey(KEY_INDEX), () -> new AlgorithmInitializationException(this, String.format("%s can not be null", KEY_INDEX)));
        return Integer.parseInt(props.getProperty(KEY_INDEX));
    }
    ...
    private byte[] createSm4Iv(final Properties props, final int sm4Mode) {
        ...
        ShardingSpherePreconditions.checkState(props.containsKey(IV), () -> new AlgorithmInitializationException(this, String.format("%s can not be null", IV)));
